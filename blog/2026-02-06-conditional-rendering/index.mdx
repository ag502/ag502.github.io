...
지금까지 살펴본 예시에서, 우리의 전략은 마크업의 일부를 선택적으로 추가하거나 제거하는 것이였습니다.

하지만 문제를 해결하기 위한 다른 방법이 있습니다. 어쩌면 이 방법이 훨씬 친숙하고 익숙할 수 있습니다.

HTML 요소 전체를 제거하거나 추가하는것 대신에, CSS를 사용하여 해당 요소의 노출여부를 제어할 수 있습니다.

리액트에서 어떻게 처리하는지 보여드리겠습니다.
...
만약 친구가 온라인 상태이면 `style`은 `undefined`가 되고 아무런 일도 일어나지 않습니다. 따라서 초록색 점은 노출됩니다.

만약 친구가 오프라인 상태이면, 초록색 점에 `display: none`이 적용되고 해당 UI는 보이지 않게 됩니다.

### 🎆 방법 비교해보기

지금까지 자바스크립트 연산자를 이용한 조건부 렌더링 방법과 CSS를 이용한 노출제어 방법을 살펴봤습니다. 어떤 방법이 더 나을까요?

사실 사용성 측면에서 봤을 때 아무런 차이가 없습니다. `display: none`으로 숨겨진 요소는 기능적으로 페이지에서 사라집니다.
시각적인 측면뿐만 아니라 키보드 탐색 및 스크린 리더 지원 측면에서도 마찬가지 입니다.

하지만 성능적인 측면에서는 흥미로운 점이 있습니다.

DOM 노드는 존재할 때 메모리를 점유합니다. 그리고 점유된 메모리는 사용자에게 보이는것과 무관하게 계속 메모리를 점유하고 있습니다.
많은 DOM 노드가 있는 대규모 애플리케이션에서, 조건부 렌더링을 사용해 DOM 노드의 수를 줄이는 것이 메모리 점유율을 줄일 수 있어 이득입니다.

반면에 새로운 DOM노드를 추가하는것이 CSS 속성을 사용해 토글하는것보다 느립니다. 만약 사용자가 콘텐츠를 토글한다면(아코디언 메뉴를 확장하는 것처럼), `display` 속성을 사용하는것이 빠를 수 있습니다.

하지만 대부분의 경우 차이가 너무 미미해서 실제로 큰 문제가 되지 않습니다.

제가 제안드리는 방법은 기본적으로 조건부 렌더링을 사용하는 것입니다. 만약 저사양 기기에서 문제가 된다면, CSS를 사용하는 방법으로 바꾼 후 테스트해보세요.

이런 상황에서는 "섣부른 최적화"의 함정에 빠지기 쉽습니다. 조심하세요!
